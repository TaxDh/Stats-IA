...
#sortie: tf.Tensor(b'Hello TensorFlow!', shape=(), dtype=string)
...
#dense veut dire que c'est tout connecté dans le réseau de neuronne
#units = 50, 50 parametres cachés
#activation 36 min, fct d'activation, doit etre appelé a chaque couche sauf la derniere couche

# Observons et tentons de comprend
NN
#le 51 vient du beta_0 en plus, donc si on fait le calcul, on a 50 et on doit ajouter beta_0


ncol(tes_x)*50 #Sont ou les 50 paramètres de plus ?
#50 parametres caché, couche chaché
...
image_1  <- as.data.frame(x_train[1, , ])#on peut changer le 1 pour 110 ou 40
...
NN_MNIST %>%
   ...
   layer_dense(units = 10, activation = "softmax")#rien quand c'est régression, softmax quand multicatégoriel
...
system.time(hist <- NN_MNIST %>% fit(x_train , y_train , epochs = 30, batch_size = 128,
          validation_split = 0.2))#encore un bogue
...
#ligne 183 ne fonctionne pas du prof