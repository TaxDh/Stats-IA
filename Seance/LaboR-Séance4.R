...
alpha <- seq(0,1,0.05)#determine une sequence entre 0 a 1, avec saut de 0.05
alpha
...
#le paramètre alpha créé un pénalité hybrid entre lasso et ridge.
cv.out <- cv.glmnet(x_train, y_train, alpha = sortie[[1]])
plot(cv.out)
bestlam <- cv.out$lambda.min
...
for (i in 1:nbs) {
  id <- sample(nrow(X),200,replace=TRUE)
  Xtrain <- X[id,]
  Ytrain <- Y[id]
